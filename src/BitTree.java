...
public class BitTree {
  ...
  public BitTreeNode setHelper(BitTreeNode node, String bits, int i, String value) {
    if (node instanceof BitTreeLeaf){
      ...
      // i know he said not to do this, but i don't understand how to access the left and right value otherwise
      BitTreeInteriorNode innerNode = (BitTreeInteriorNode) node;
      if (bits.charAt(i) == '0') {
        if (node == null) {
          innerNode.left = new BitTreeInteriorNode(null);
          setHelper(innerNode.left, bits, i ++, value);
        }
      } else if (bits.charAt(i) == '1') {
        if (node == null) {
          innerNode.right = new BitTreeInteriorNode(null);
          setHelper(innerNode.right, bits, i ++, value);
        }
      }
